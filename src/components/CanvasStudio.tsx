...
import { MousePointer2, PenTool, Type, Printer, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col w-full h-full bg-white border border-[#E2E8F0] rounded-lg overflow-hidden shadow-sm">
      <div className="flex items-center gap-3 p-3 border-b border-[#E2E8F0] bg-[#FAF9F8]">
        <button onClick={() => { if(canvasRef.current) canvasRef.current.isDrawingMode = false }} className="p-2 hover:bg-white rounded border border-transparent hover:border-gray-200 transition-all">
        ...
        <button onClick={downloadPDF} className="bg-[#0078D4] hover:bg-[#106EBE] text-white px-4 py-1.5 rounded flex items-center gap-2 text-sm font-medium transition-all">
      ...
      <div className="relative flex-1 overflow-auto bg-[#F3F2F1] p-8 flex justify-center items-start min-h-[500px]">
        {loading && <div className="absolute inset-0 flex items-center justify-center bg-white/50 z-10"><Loader2 className="animate-spin text-[#0078D4]" size={40} /></div>}
