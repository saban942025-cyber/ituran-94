...
// תיקון נתיב הייבוא בהתאם למבנה התיקיות במאגר שלך
import { db } from '../lib/firebase'; 
import { ref, onValue, push, set, remove } from 'firebase/database';
import { 
  Package, Database, Search, AlertCircle, X, ExternalLink, CheckCircle2, Trash2
} from 'lucide-react';
      ...
      {showAdmin && (
        <div className="fixed inset-0 bg-blue-950/40 backdrop-blur-md z-[2000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3.5rem] w-full max-w-2xl shadow-2xl p-10 border-t-[12px] border-blue-900 relative">
            ...
            <h3 className="text-2xl font-black mb-6 flex items-center gap-3 text-blue-900"><Database size={26}/> הזרקת נתוני קופיילוט</h3>
            <textarea 
              value={jsonInput} 
              onChange={(e) => setJsonInput(e.target.value)} 
              className="w-full h-80 p-6 font-mono text-[11px] border-2 rounded-[3rem] bg-slate-50 mb-6 outline-none focus:border-blue-500 shadow-inner" 
              ...
              onClick={async () => {
                try {
                  ...
                  for (const item of items) { 
                    await push(ref(db, 'delivery_history'), { ...item, date: item.date || selectedDate }); 
                  }
                  setJsonInput(''); setShowAdmin(false); alert('הארכיון עודכן בהצלחה!');
                } catch { alert('JSON לא תקין'); }
              }} 
              className="w-full py-5 bg-blue-900 text-white rounded-[2rem] font-black text-lg shadow-xl hover:bg-black transition-all"
            >
              בצע עדכון ארכיון
            </button>
          </div>
